    ...
    <section className="w-full">
      ...
      <section className="w-full flex my-12">
        {daysOfWeek.map((day) => (
          <span
            key={day}
            className="w-1/7 font-bold uppercase text-gray-800 tracking-wider flex justify-center text-sm sm:text-base"
          >
            {day}
          </span>
        ))}
      ...
      <section className="flex flex-wrap">
        {[...Array(firstDayOfMonth).keys()].map((_, index) => (
          <span
            key={`empty-${index}`}
            className="w-1/7 aspect-square flex justify-center text-gray-800 text-base sm:text-lg cursor-pointer hover:bg-gray-100 rounded"
          />
        ))}
