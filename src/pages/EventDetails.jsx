...
//import { getSchedules } from "../backend/api/EventSchedule"; // <-- make sure this exists
// import { getDocuments } from "../backend/api/EventDocuments"; // later if backend ready
...
export default function EventDetails() {
  ...
  const [documents, setDocuments] = useState([]); // backend later
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [guestsData, vendorsData, schedulesData] = await Promise.all([
          getGuests(event._id),
          getVendors(event._id),
          //getSchedules(event._id),
          // getDocuments(event._id)
        ]);
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-sky-100 to-green-900 pb-8">
      ...
      <section className="p-6 w-10/12 mx-auto bg-white rounded-lg shadow mt-8">
        ...
        <span className={`px-3 py-1 rounded-full text-xs mb-2 ${event.type === "Wedding" ? "bg-pink-500 text-white" : "bg-gray-300"}`}>
          {event.type}
        </span>
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-8">
          ...
          {/* Venues */}
          <section className="bg-red-100 rounded-lg shadow p-6 cursor-pointer hover:bg-red-200 transition" onClick={() => setShowVenuesModal(true)}>
          ...
          {/* Schedule */}
          <section className="bg-yellow-100 rounded-lg shadow p-6 cursor-pointer hover:bg-yellow-200 transition" onClick={() => setShowScheduleModal(true)}>
            <h3 className="text-lg font-semibold mb-2">Schedule</h3>
            {/* <p className="text-xs mb-2">{schedules.length} items planned</p> */}
            <p className="text-xs text-yellow-900">Click to view schedule</p>
          </section>

          {/* Floor Plan */}
          <section className="bg-pink-100 rounded-lg shadow p-6 cursor-pointer hover:bg-pink-200 transition" onClick={() => setShowFloorPlanModal(true)}>
        ...
        {/* Modals */}
        {showEditEventModal && <EditEventModal event={event} onClose={() => setShowEditEventModal(false)} onSave={handleEditEventSave} />}
        {showRSVPModal && <RSVPModal guests={guests} onClose={() => setShowRSVPModal(false)} />}
        {showAddGuestsModal && <AddGuestsModal eventId={event._id} onClose={() => setShowAddGuestsModal(false)} />}
        {showVendorsModal && <VendorsModal vendors={vendors} eventId={event._id} onClose={() => setShowVendorsModal(false)} />}
        {showVenuesModal && <VenuesModal onClose={() => setShowVenuesModal(false)} onAddVenue={() => setShowAddVenuesModal(true)} />}
        {showAddVenuesModal && <AddVenuesModal eventId={event._id} onClose={() => setShowAddVenuesModal(false)} />}
        {showScheduleModal && <ScheduleModal schedules={schedules} onClose={() => setShowScheduleModal(false)} onAddSchedule={() => setShowAddScheduleModal(true)} />}
        {showAddScheduleModal && <AddScheduleModal eventId={event._id} onClose={() => setShowAddScheduleModal(false)} />}
        {showFloorPlanModal && <FloorPlanModal floorPlanUrl={event.floorPlanUrl} onClose={() => setShowFloorPlanModal(false)} />}
        {showDocumentsModal && <DocumentsModal documents={documents} onClose={() => setShowDocumentsModal(false)} />}
      </section>
    </section>
  );
}
